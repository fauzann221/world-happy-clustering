...
# Sidebar
st.sidebar.write("### Nilai jumlah K")
clust = st.sidebar.slider("Tentukan Jumlah Cluster : ", 2,10,3,1)
st.sidebar.divider()
st.sidebar.write('''
 GDP perkapita adalah besarnya GDP riil dibagi jumlah penduduk. Dari penjelasan ini diketahui bahwa GDP perkapita mengukur berapa rata- rata barang dan jasa yang dapat dikonsumsi penduduk suatu negara.
''')
...
def k_means(n_clust):
    ...
    plt.title(f"Cluster hubungan antara Happiness Score dan GDP per capita")
    ...
    for label in x['Labels']:
        plt.annotate (label,
            ...
            size = 20, weight='bold',
            color = 'black')            
    st.write("### Cluster Plot Happiness Score & GDP : ")
    plot = plt.gcf() 
    st.pyplot(plot)  
...
st.write("### Hubungan antara GDP dengan happiness score")
...
plt.title('Hubungan antara GDP per capita and Happiness Score')
st.pyplot()

st.write('''
    Dari grafik diatas dapat kita simpulkan ada hubungan positif antara GDP per kapita dan skor kebahagiaan. Artinya, negara-negara dengan GDP per kapita yang lebih tinggi cenderung memiliki skor kebahagiaan yang lebih tinggi.
''')
